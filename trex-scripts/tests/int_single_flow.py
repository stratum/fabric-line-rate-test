...
import logging
from lib.xnt import get_readable_int_report_str, IntL45ReportFixed
...
class IntSingleFlow(BaseTest):
    ...
    def start(self) -> None:
        pkt = Ether(src=SOURCE_MAC, dst=DEST_MAC) / \
              IP(src=SOURCE_IP, dst=DEST_IP) / \
              UDP(sport=1234, dport=4567)/('*' * 1500)
        ...
        logging.info(
            'Start capturing first %s RX packet from INT collector',
            pkt_capture_limit)
        self.stl_client.set_service_mode(
            ports=INT_COLLECTPR_PORTS, enabled=True)
        capture = self.stl_client.start_capture(
            rx_ports=INT_COLLECTPR_PORTS, limit=pkt_capture_limit,
            bpf_filter='udp and dst port 32766')

        logging.info('Starting traffic, duration: %ds, throughput: %s',
                     self.duration, self.mult)
        ...
        if num_pkts not in range(self.duration - 1, self.duration + 2):
            logging.error('Expected to receive %d +/- 1 pakcets, but got %d',
                          self.duration, num_pkts)
        ...
        for pkt_info in output:
            ...
            if IntL45ReportFixed in pkt:
                ...
                if prev_seq_no and seq_no != (prev_seq_no + 1):
                    logging.error('Expect to get seq no %d, but got %d',
...
def get_test(stl_client: STLClient, duration: int = 1,
             mult: str = '10gbpsl1') -> IntSingleFlow:
