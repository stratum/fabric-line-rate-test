...
def get_readable_port_stats(port_stats: str) -> str:
    ...
    RX util: {}'''.format(
        opackets,
        ipackets,
        obytes, to_readable(obytes, 'Bytes'),
        ibytes, to_readable(ibytes, 'Bytes'),
        oerrors,
        ierrors,
        tx_bps, to_readable(tx_bps),
        tx_pps, to_readable(tx_pps, 'pps'),
        tx_bps_L1, to_readable(tx_bps_L1),
        tx_util,
        rx_bps, to_readable(rx_bps),
        rx_pps, to_readable(rx_pps, 'pps'),
        rx_bps_L1, to_readable(rx_bps_L1),
        rx_util
    )
